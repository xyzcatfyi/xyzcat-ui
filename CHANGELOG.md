...
- `AuthLogin` — password gate component, localStorage-based, configurable via props
- `AuthLogout` — logout button, clears localStorage auth state

### Milestone

- Phase 2 core components complete — 15 components total shipped:
  `ToggleDarkLight` · `ButtonPrimary` · `ButtonSecondary` · `ButtonDestructive` ·
  `LayoutHeaderText` · `LayoutFooter` · `NavBar` · `NavHamburger` ·
  `SettingsSection` · `SettingsRow` · `ModalConfirm` · `DataExportImport` ·
  `IconPlaceholder` · `AuthLogin` · `AuthLogout`
